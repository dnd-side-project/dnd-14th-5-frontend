...
## 5-1) 디자인 토큰 규칙
- 모든 디자인 작업은 **디자인 토큰을 기준**으로 작성합니다.
- 색상/타이포/간격 등은 임의 값 대신 **정의된 토큰만 사용**합니다.
- 새로운 값이 필요하면 **토큰을 추가**한 뒤 사용하며, 관련 문서를 함께 업데이트합니다.
- 디자인 토큰 정의 위치:
...
- `src/**` 내부의 코드는 TypeScript만 허용합니다.
  - 허용: `.ts`, `.tsx`
  - 금지: `.js`, `.jsx`, `.mts`
  - **리뷰 안내**: `src/`에서의 “코드”는 JS/TS 소스 파일을 의미합니다.  
    CSS/이미지/폰트 등 **정적 자산 파일은 허용**되며, JS/JSX 대신 **TS/TSX 사용**을 요구합니다.
